...
import { calculateSemesterStats, calculateOverallStats, calculateOverallStatsWithFinalExams } from './utils/calculation';
...
const App: React.FC = () => {
  ...
  const handleUpdateFinalExam = (subjectId: string, data: { 
    mode: 'grade' | 'points'; 
    grade?: number; 
    ...
    setConfig(prev => {
      ...
      return {
        ...
        subjects: prev.subjects.map(s => {
          ...
          return {
             ...s,
             finalExamInputMode: data.mode,
             globalFinalExamGrade: data.mode === 'grade' ? data.grade : s.globalFinalExamGrade,
             finalExamPointsAchieved: data.mode === 'points' ? data.pointsAchieved : s.finalExamPointsAchieved,
             finalExamPointsMax: data.mode === 'points' ? data.pointsMax : s.finalExamPointsMax
          };
        })
      };
    });
                      ...
                      {temp.disclaimer && (
                        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                          <p className="text-[10px] text-amber-600 dark:text-amber-400 flex items-start gap-1">
                            ...
                            <span>Berechnungen können Abweichungen enthalten unbedingt endnote manuell überprüfen!</span>
                          </p>
                        </div>
                      )}
                   </div>
  ...
  const overallWithFinalExams = calculateOverallStatsWithFinalExams(config);
        ...
        {viewMode === 'settings' ? (
          ...
          <div className="space-y-6 sm:space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Overall Stats with Final Exams */}
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-3 mt-1">
                          <div className="text-4xl sm:text-5xl font-black text-gray-900 dark:text-white">
                            {overallWithFinalExams.averageWithFinalExams.toFixed(2)}
                          </div>
                          {overallWithFinalExams.averageWithoutFinalExams !== overallWithFinalExams.averageWithFinalExams && (
                            <div className="text-sm text-gray-400 font-medium">
                              <span className="text-[10px] uppercase">ohne AP:</span> {overallWithFinalExams.averageWithoutFinalExams.toFixed(2)}
                            </div>
                          )}
                        </div>
                    </div>
                    <div className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 ${overallWithFinalExams.averageWithFinalExams >= 4 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
                         {overallWithFinalExams.averageWithFinalExams >= 4 ? <CheckCircle2 size={14}/> : <AlertCircle size={14}/>}
                         {overallWithFinalExams.averageWithFinalExams >= 4 ? t.passed : t.failed}
                    </div>
                 </div>
