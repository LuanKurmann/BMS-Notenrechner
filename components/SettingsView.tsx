...
const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const handleBetaFeaturesToggle = () => {
    onUpdateSettings({ ...config.settings, enableBetaFeatures: !config.settings.enableBetaFeatures });
  };
  ...
  const handleAIConsentConfirm = async () => {
    ...
    try {
      ...
      const grades = await analyzeGradeImage(pendingFile, config);
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-20">
      ...
      {/* AI Import Section - Only visible when beta features enabled */}
      {config.settings.enableBetaFeatures && (
      ...
      <AIConsentModal 
        ...
        message={t.aiConsent.message}
        confirmText={t.aiConsent.confirm}
        cancelText={t.aiConsent.cancel}
      />
      ...
      <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <div className="mb-8">
            <label className="block text-gray-900 dark:text-white font-bold mb-2">{t.settingsView.betaFeatures}</label>
            <p className="text-sm text-gray-500 mb-4">{t.settingsView.betaFeaturesDesc}</p>
            <button
              onClick={handleBetaFeaturesToggle}
            ...
            >
              {config.settings.enableBetaFeatures ? '✓ Aktiviert' : 'Deaktiviert'}
            </button>
        </div>
