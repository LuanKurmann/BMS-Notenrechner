...
import { calculateSemesterStats, calculateSubjectAverage, calculateOverallStatsWithFinalExams, roundGrade } from '../utils/calculation';
import { TEXTS } from '../constants';
import { TrendingUp, TrendingDown, Award, Book, Hash, GraduationCap, Calculator } from 'lucide-react';
...
const StatsView: React.FC<StatsViewProps> = ({ config, activeSemesterId, lang }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      {/* Final Exam Overview Section */}
      <FinalExamOverview config={config} t={t} />

    </div>
  );
};

// Separate component for Final Exam Overview - Shows ALL subjects
const FinalExamOverview: React.FC<{ config: SchoolConfig; t: typeof TEXTS['de'] }> = ({ config, t }) => {
  const finalExamStats = calculateOverallStatsWithFinalExams(config);
  
  // Show all subjects, not just those with final exams
  const allSubjects = finalExamStats.subjectDetails;
  
  // Check if we have any grades at all
  const hasAnyGrades = allSubjects.some(s => s.averageWithout !== null);
  if (!hasAnyGrades) return null;

  // Check if any subject has a final exam (to show the comparison columns)
  const hasAnyFinalExam = allSubjects.some(s => s.hasFinalExam);

  return (
    <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-800 p-5 rounded-3xl shadow-sm border border-indigo-100 dark:border-gray-700">
      <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
        <GraduationCap size={18} className="text-indigo-600 dark:text-indigo-400" /> {t.stats.finalExamOverview}
      </h3>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-700 p-4 rounded-2xl text-center">
          <div className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">
            {t.stats.avgWithoutFinalExams}
          </div>
          <div className="text-3xl font-black text-gray-800 dark:text-white">
            {finalExamStats.averageWithoutFinalExams > 0 ? (
              <>
                {roundGrade(finalExamStats.averageWithoutFinalExams, '0.5').toFixed(1)}
                <span className="text-sm font-normal text-gray-400 ml-1">({finalExamStats.averageWithoutFinalExams.toFixed(2)})</span>
              </>
            ) : '-'}
          </div>
        </div>
        <div className="bg-white dark:bg-gray-700 p-4 rounded-2xl text-center border-2 border-indigo-200 dark:border-indigo-900">
          <div className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase mb-2 flex items-center justify-center gap-1">
            <Calculator size={12} /> {t.stats.avgWithFinalExams}
          </div>
          <div className={`text-3xl font-black ${
            finalExamStats.averageWithFinalExams >= 4 
              ? 'text-indigo-600 dark:text-indigo-400' 
              : 'text-red-500'
          }`}>
            {finalExamStats.averageWithFinalExams > 0 ? (
              <>
                {roundGrade(finalExamStats.averageWithFinalExams, '0.5').toFixed(1)}
                <span className="text-sm font-normal text-gray-400 ml-1">({finalExamStats.averageWithFinalExams.toFixed(2)})</span>
              </>
            ) : '-'}
          </div>
        </div>
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">
          {t.stats.subjectOverall}
        </h4>
        {allSubjects.map(subject => (
          <div 
            key={subject.id} 
            className={`bg-white dark:bg-gray-700 p-3 rounded-xl flex items-center justify-between gap-3 ${
              !subject.hasFinalExam ? 'opacity-80' : ''
            }`}
          >
            <div className="flex-1 min-w-0">
              <div className="font-medium text-gray-800 dark:text-white text-sm truncate flex items-center gap-2">
                {subject.name}
                {subject.hasFinalExam && (
                  <span className="text-[9px] bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded font-bold">
                    AP
                  </span>
                )}
              </div>
              {subject.hasFinalExam ? (
                <div className="text-[10px] text-gray-400 mt-0.5">
                  {t.finalExam}: {subject.finalExamWeight}% 
                  {subject.finalExamGrade !== undefined 
                    ? ` → ${roundGrade(subject.finalExamGrade, '0.5').toFixed(1)}` 
                    : ` (${t.stats.noFinalExamYet})`
                  }
                </div>
              ) : (
                <div className="text-[10px] text-gray-400 mt-0.5">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {/* Without Final / Only Average */}
              <div className="text-center px-2">
                <div className="text-[9px] text-gray-400 uppercase">
                  {subject.hasFinalExam ? 'Ohne' : 'Ø'}
                </div>
                <div className={`text-sm font-bold ${
                  subject.averageWithout !== null && subject.averageWithout < 4 
                    ? 'text-red-500' 
                    : 'text-gray-600 dark:text-gray-300'
                }`}>
                  {subject.averageWithout !== null ? (
                      <>
                        {roundGrade(subject.averageWithout, '0.5').toFixed(1)}
                        <div className="text-[8px] font-normal text-gray-400">({subject.averageWithout.toFixed(2)})</div>
                      </>
                    ) : '-'}
                </div>
              </div>
              {/* Arrow & With Final - only for subjects with final exam */}
              {subject.hasFinalExam && (
                <>
                  <div className="text-gray-300 dark:text-gray-600">→</div>
                  <div className="text-center px-2">
                    <div className="text-[9px] text-indigo-500 uppercase">Mit</div>
                    <div className={`text-sm font-bold ${
                      subject.finalExamGrade !== undefined 
                        ? (subject.averageWith !== null && subject.averageWith >= 4 
                            ? 'text-indigo-600 dark:text-indigo-400' 
                            : 'text-red-500')
                        : 'text-gray-400'
                    }`}>
                      {subject.averageWith !== null ? (
                          <>
                            {roundGrade(subject.averageWith, '0.5').toFixed(1)}
                            <div className="text-[8px] font-normal text-gray-400">({subject.averageWith.toFixed(2)})</div>
                          </>
                        ) : '-'}
                    </div>
                  </div>
