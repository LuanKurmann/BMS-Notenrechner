...
import { Plus, Calendar, ArrowLeft, Trash2, GraduationCap, Calculator, CheckCheck, Hash } from 'lucide-react';
import { Subject, Grade, ComingGrade } from '../types';
import { TEXTS } from '../constants';
import { calculateSubjectAverage, getEffectiveFinalExamGrade, pointsToGrade } from '../utils/calculation';
...
interface SubjectDetailProps {
  ...
  onUpdateFinalExam: (subjectId: string, data: { 
    mode: 'grade' | 'points'; 
    grade?: number; 
  ...
  return (
    <div className="animate-fade-in pb-20">
      ...
      <div className="space-y-6 sm:space-y-8">
          ...
          {/* Final Exam Section - Global across all semesters */}
          {subject.hasFinalExam && (
            <div className="bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-gray-800 dark:to-gray-800 p-4 sm:p-6 rounded-3xl shadow-sm border border-indigo-100 dark:border-gray-700">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                      {lang === 'de' ? 'Gewichtung' : lang === 'fr' ? 'Pondération' : 'Weight'}: {subject.finalExamWeight}%
                    </p>
                    <p className="text-xs text-indigo-500 dark:text-indigo-400 font-medium mt-1">
                      {lang === 'de' ? '→ Gilt für alle Semester' : lang === 'fr' ? '→ S\'applique à tous les semestres' : '→ Applies to all semesters'}
                    </p>
                </div>
              </div>
              ...
              <div className="flex gap-2 mb-4">
                <button
                  onClick={() => onUpdateFinalExam(subject.id, { mode: 'grade', grade: subject.globalFinalExamGrade })}
                  className={`flex-1 py-2 px-3 rounded-xl text-sm font-bold transition-all ${
                    subject.finalExamInputMode !== 'points'
                      ? 'bg-indigo-600 text-white'
                      : 'bg-white dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-600'
                  }`}
                >
                  {lang === 'de' ? 'Note' : lang === 'fr' ? 'Note' : 'Grade'}
                </button>
                <button
                  onClick={() => onUpdateFinalExam(subject.id, { 
                    mode: 'points', 
                    pointsAchieved: subject.finalExamPointsAchieved, 
                    pointsMax: subject.finalExamPointsMax 
                  })}
              ...
              {subject.finalExamInputMode !== 'points' ? (
                <div className="flex justify-center">
                  <input 
                    ...
                    step="0.1"
                    placeholder="-"
                    value={subject.globalFinalExamGrade || ''}
                    onChange={(e) => onUpdateFinalExam(subject.id, { 
                      mode: 'grade', 
                      grade: e.target.value ? parseFloat(e.target.value) : undefined 
                    })}
                    className="w-full sm:w-32 h-14 sm:h-16 text-2xl text-center font-bold rounded-2xl border-2 border-indigo-100 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-4 focus:ring-indigo-100 dark:focus:ring-gray-600 outline-none transition-all placeholder-gray-300"
                  />
                </div>
              ) : (
                /* Points Input */
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <div className="flex-1">
                      <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1 block">
                        {lang === 'de' ? 'Erreichte Punkte' : lang === 'fr' ? 'Points obtenus' : 'Points achieved'}
                      </label>
                      <input 
                        type="number"
                        min="0"
                        step="0.5"
                        placeholder="0"
                        value={subject.finalExamPointsAchieved ?? ''}
                        onChange={(e) => onUpdateFinalExam(subject.id, { 
                          mode: 'points', 
                          pointsAchieved: e.target.value ? parseFloat(e.target.value) : undefined,
                          pointsMax: subject.finalExamPointsMax
                        })}
                        className="w-full h-12 text-xl text-center font-bold rounded-xl border-2 border-indigo-100 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-4 focus:ring-indigo-100 dark:focus:ring-gray-600 outline-none transition-all placeholder-gray-300"
                      />
                    </div>
                    <div className="text-2xl font-bold text-gray-300 dark:text-gray-600 pt-5">/</div>
                    <div className="flex-1">
                      <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1 block">
                        {lang === 'de' ? 'Max. Punkte' : lang === 'fr' ? 'Points max.' : 'Max. points'}
                      </label>
                      <input 
                        type="number"
                        min="1"
                        step="1"
                        placeholder="100"
                        value={subject.finalExamPointsMax ?? ''}
                        onChange={(e) => onUpdateFinalExam(subject.id, { 
                          mode: 'points', 
                          pointsAchieved: subject.finalExamPointsAchieved,
                          pointsMax: e.target.value ? parseFloat(e.target.value) : undefined
                        })}
                        className="w-full h-12 text-xl text-center font-bold rounded-xl border-2 border-indigo-100 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-4 focus:ring-indigo-100 dark:focus:ring-gray-600 outline-none transition-all placeholder-gray-300"
                      />
                    </div>
                  ...
                  {subject.finalExamPointsAchieved !== undefined && subject.finalExamPointsMax !== undefined && subject.finalExamPointsMax > 0 && (
                    <div className="bg-white dark:bg-gray-700 p-3 rounded-xl text-center border border-indigo-100 dark:border-gray-600">
                      <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">
                        {lang === 'de' ? 'Berechnete Note' : lang === 'fr' ? 'Note calculée' : 'Calculated grade'}: 
                      </span>
                      <span className={`ml-2 text-xl font-black ${
                        pointsToGrade(subject.finalExamPointsAchieved, subject.finalExamPointsMax, subject.finalExamRounding || subject.rounding) >= 4 
                          ? 'text-indigo-600 dark:text-indigo-400' 
                          : 'text-red-500'
                      }`}>
                        {pointsToGrade(subject.finalExamPointsAchieved, subject.finalExamPointsMax, subject.finalExamRounding || subject.rounding).toFixed(2)}
                      </span>
                      <span className="text-xs text-gray-400 ml-2">
                        ({subject.finalExamPointsAchieved} × 5 / {subject.finalExamPointsMax} + 1)
                      </span>
                    </div>
                  )}
                </div>
